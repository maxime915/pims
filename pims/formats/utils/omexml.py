...
<!-- Warning: this comment is an OME-XML metadata block, which contains crucial dimensional parameters and other 
important metadata. Please edit cautiously (if at all), and back up the original data before doing so. 
For more information, see the OME-TIFF web site: http://ome-xml.org/wiki/OmeTiff. -->
...
class OMEXML(object):
    ...
    def __init__(self, xml=None, rootnode=None):
        ...
        ome_elem = self.dom
        if not ome_elem.get('UUID'):
            ome_elem.set('UUID', 'urn:uuid:' + str(uuid.uuid4()))
        self.uuidStr = ome_elem.get('UUID')
    ...
    class Pixels(object):
        ...
        def TiffData(self, index=0):
            ...
            return OMEXML.TiffData(tiff_data)
    ...
    class Plate(object):
        ...
        https://www.openmicroscopy.org/Schemas/SPW/2007-06/
        """
